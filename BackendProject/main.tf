...
resource "aws_instance" "backend_proj_server" {                            # BLOCK
  ...
  tags = {
    Name = "Backend Project EC2 Server"
  }
}
...
resource "aws_dynamodb_table" "dynamodb-state-lock" {
  name         = "tf-state-lock-table-${random_id.randomness.hex}"
